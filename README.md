...
__Основные задачи__:
  ...
  :exclamation: Будьте внимательны при работе с краями изображения. Чтобы к
  этим областям приложить ядро фильтра и посчитать отклик, необходимо [
  экстраполировать](http://docs.opencv.org/3.0-beta/modules/imgproc/doc/filtering.html) граничные значения пикселей.
  Фильтры среднего по окрестности и собеля используют `BORDER_REFLECT_101`
  экстраполяцию, линейный фильтр использует `BORDER_CONSTANT` = 0, медианный
  фильтр использует только значения внутри изображения.
